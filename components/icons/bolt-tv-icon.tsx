import styles from "./bolt-tv-icon.module.css";

type BoltTvIconProps = Omit<React.SVGAttributes<SVGSVGElement>, "fill"> & {
  "aria-label"?: string;
};

export const boltShimmerTextClass = styles.shimmerText;

export function BoltTvIcon({
  className,
  "aria-label": ariaLabel = "Lightning",
  width = 14,
  height = 14,
  ...props
}: BoltTvIconProps) {
  return (
    <svg
      ...
      viewBox="0 0 18 18"
      fill="currentColor"
      className={["text-amber-500", className].filter(Boolean).join(" ")}
      {...props}
    >
